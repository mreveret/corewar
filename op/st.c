...
void	log_st(t_list *list)
{
	printf("P %4d | st r%d %s%d\n", ((t_p *)list->content)->id, ((t_p *)list->content)->reg_num[0],
			...
			((t_p *)list->content)->arg[1] : ((t_p *)list->content)->reg_num[1]));
}
...
void	op_st(t_list *list, t_vm *x)
{
	...
	{
		tmp_pc = move_pc(((t_p *)list->content)->pc - 1, ((t_p *)list->content)->arg[1] % IDX_MOD);
	...
	{
		if (((t_p *)list->content)->arg[1] >= REG_NUMBER || ((t_p *)list->content)->arg[1] < 0)
			return ;
		((t_p *)list->content)->reg[((t_p *)list->content)->arg[1]] = ((t_p *)list->content)->arg[0];
	}
	else
		return ;
}
